...
export class LineNode extends ParagraphNode {
  ...
  getType() {
    return `${LineNode.getType()} [${this.__elementType}${this.__forced ? "*" : ""}]`;
  }

  createDOM(config: EditorConfig): HTMLElement {
    const dom: HTMLParagraphElement = super.createDOM(config) as HTMLParagraphElement;
  ...
  updateDOM(prevNode: LineNode, dom: HTMLElement, config: EditorConfig): boolean {
  ...
  static importDOM(): DOMConversionMap | null {
    ...
    function convertLineElement(node: HTMLElement) {
      ...
      const lineNode = $createLineNode(type.replace(/^line-/, "") as LineNodeType);
  ...
  canMergeWith(node: ElementNode): boolean {
    return node instanceof LineNode && this.getElementType() === node.getElementType();
  }
      ...
      if (
        ...
        if (currentType === LineNodeType.None) {
          ...
          if (this.getChildAtIndex(0) === anchorNode) {
            ...
            const [mark, content] = this.getChildAtIndex(0)?.splitText(anchorOffset);
            mark.remove();
  ...
  impliedType(): LineNodeType {
    ...
    if (currentType !== LineNodeType.None) {
      ...
      if (currentType === LineNodeType.Dialog && currentText.match(PARENTHETICAL_PATTERN)) {
    ...
    if (currentType !== LineNodeType.Scene && currentText.match(SCENE_HEADER_PATTERN)) {
  ...
  checkForLostType(_anchorNode: LexicalNode, _anchorOffset: number): boolean {
    ...
    if (this.getElementType() === LineNodeType.Parenthetical && this.wouldRequireForce()) {
  ...
  wouldRequireForce() {
    ...
    if (this.getElementType() === LineNodeType.Lyric && !this.getTextContent().startsWith("~")) {
      return true;
    }
