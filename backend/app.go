...
func (a *App) Startup(ctx context.Context) {
	...
	runtime.EventsOn(ctx, "sceneContentChange", func(data ...interface{}) {
		runtime.LogPrint(ctx, "~~~Scene Content Change~~~~")
		sceneMap := data[0].(map[string]interface{})
		typedMap := make(map[string]string)
		for k, v := range sceneMap {
			typedMap[k] = v.(string)
		}
		a.ai.UpdateSceneContent(ctx, typedMap)
	})
